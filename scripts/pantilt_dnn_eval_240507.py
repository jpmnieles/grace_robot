...
class VisuoMotorNode(object):
    ...
    def __init__(self, num_trials, motors=["EyeTurnLeft", "EyeTurnRight", "EyesUpDown",
                                                      "NeckRotation", "UpperGimbalLeft", "UpperGimbalRight",
                                                      "LowerGimbalLeft", "LowerGimbalRight"], degrees=True):
        # DNN Loading
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.model = GraceLitModule.load_from_checkpoint("epoch_041.ckpt")
        self.model.eval()
    ...
    def eye_imgs_callback(self, left_img_msg, right_img_msg, chest_img_msg, depth_img_msg):
        ...
        if elapsed_time > 1000e-3:
            ...
            # Pan Target
            if self.ctr%2 == 0:
                self.chess_idx = random.randint(0,44)
            ...
            if target_z != 0.3:
                ...
                # Output of DNN
                in_tensor = torch.tensor([[x,y,z,0,0,0]], dtype=torch.float32).to(self.device)
                with torch.no_grad():
                    output = self.model(in_tensor)
                    output = output.squeeze().cpu().numpy()
                    theta_l_pan = np.degrees(output[0])
                    theta_tilt = np.degrees(output[1])
            ...
            if self.ctr==self.num_ctr:
                ...
                results_dir = os.path.join(parent_dir, 'results','pantilt_dnn_eval')
...
if __name__ == '__main__':
    ...
    vismotor = VisuoMotorNode(num_trials=500)
    rospy.spin()
