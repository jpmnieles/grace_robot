...
class VisuoMotorNode(object):
    ...
    rl_state = {  # RL environment on the network takes care of the other side
        ...
        'theta_upper_tilt_left': None,
        'theta_upper_tilt_right': None
    }
    ...
    def eye_imgs_callback(self, left_img_msg, right_img_msg, chest_img_msg, depth_img_msg):
        ...
        if elapsed_time > 3.0:
            ...
            theta_upper_tilt_left_list = list(range(0,16,5))
            theta_upper_tilt_right_list = list(range(0,-16,-5))
            repetition = 1
            
            # print('debug:', ((self.ctr//2)//(len(theta_l_pan_list)*repetition))%(len(theta_tilt_list)))
            theta_upper_tilt_left = theta_upper_tilt_left_list[((self.ctr//2)//(len(theta_neck_pan_list)*repetition))%(len(theta_upper_tilt_left_list))]
            theta_upper_tilt_right = theta_upper_tilt_right_list[((self.ctr//2)//(len(theta_neck_pan_list)*repetition))%(len(theta_upper_tilt_right_list))]
            
            if self.ctr % 2 == 1:
                theta_neck_pan = theta_neck_pan_list[(self.ctr//2)%(len(theta_neck_pan_list))]
            else:
                theta_neck_pan = -35
                theta_upper_tilt_left = 57
                theta_upper_tilt_right = -57
            
            print('debug:', theta_neck_pan, theta_upper_tilt_left, theta_upper_tilt_right)
            
            self.move((0, 0, 0,
                       theta_neck_pan, theta_upper_tilt_left, theta_upper_tilt_right,
                       0, 0))
            with self.buffer_lock:
                ...
                self.rl_state['theta_upper_tilt_left_cmd'] = theta_upper_tilt_left
                self.rl_state['theta_upper_tilt_right_cmd'] = theta_upper_tilt_right
            ...
            if self.ctr==self.num_ctr:
                ...
                results_dir = os.path.join(parent_dir, 'results','head_panuppertilt_baseline_sweep')
                ...
                title_str = 'head_panuppertilt_baseline_sweep'
