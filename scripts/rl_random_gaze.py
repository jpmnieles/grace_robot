    ...
    def eye_imgs_callback(self, left_img_msg, right_img_msg, chest_img_msg, depth_img_msg):
        ...
        if elapsed_time > 285e-3:
            ...
            if (theta_l_pan is not None) or (theta_r_pan is not None) or (theta_tilt is not None):
                theta_l_pan = random.uniform(-18, 18)
                theta_r_pan = copy.deepcopy(theta_l_pan)
                theta_tilt = random.uniform(-31, 22)
            ...
            if self.ctr==self.num_ctr:
                ...
                results_dir = os.path.join(parent_dir, 'results','rl_random')
...
if __name__ == '__main__':
    ...
    vismotor = VisuoMotorNode(num_ctr=3000+2)
    rospy.spin()
