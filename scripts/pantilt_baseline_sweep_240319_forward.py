    ...
    def eye_imgs_callback(self, left_img_msg, right_img_msg, chest_img_msg, depth_img_msg):
        ...
        if elapsed_time > 500e-3:
            ...
            if (theta_l_pan is not None) or (theta_r_pan is not None) or (theta_tilt is not None):
                theta_l_pan_list = [-18] + list(range(0,13))
                theta_r_pan_list = [18] + list(range(-12,1))
                theta_tilt_list = list(range(0,-31,-5))
                repetition = 10
                
                # print('debug:', ((self.ctr//2)//(len(theta_l_pan_list)*repetition))%(len(theta_tilt_list)))
                theta_tilt_ovr = theta_tilt_list[((self.ctr)//(len(theta_l_pan_list)*repetition))%(len(theta_tilt_list))]
                theta_l_pan = theta_l_pan_list[(self.ctr)%(len(theta_l_pan_list))]
                theta_r_pan = theta_r_pan_list[(self.ctr)%(len(theta_r_pan_list))]
            ...
            if self.ctr==self.num_ctr:
                ...
                results_dir = os.path.join(parent_dir, 'results','pantilt_baseline_sweep_forward')
                ...
                title_str = 'pantilt_baseline_sweep_forward'
...
if __name__ == '__main__':
    ...
    vismotor = VisuoMotorNode(chess_seq=[0,2,4,6,8,6,4,2],num_trials=1042)
    rospy.spin()
