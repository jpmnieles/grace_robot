...
class VisuoMotorNode(object):
    ...
    def pos_to_target(self, pos, speed):
        msg = Target()
        msg.x = max(0.3, pos.x)
    ...
    def SetHeadFocus(self, pos, speed, ts, frame_id='robot', proportional=True, head_yaw_movement=0.6, head_pitch_movement=0.6):
        try:
            pos = self.get_blender_pos(pos, ts, frame_id)
            msg = self.pos_to_target(pos, speed)
            if proportional:
    ...
    def UpdateGaze(self, pos, ts, frame_id="realsense"):
        ...
        self.SetHeadFocus(pos, 1, ts, frame_id)
    ...
    def get_pixel_depth(self, px, depth_img, z_replace=1.0):
        u = round(px[0])
        v = round(px[1])
        ...
        if elapsed_time > 485e-3:
            ...
            if (theta_l_pan is not None) or (theta_r_pan is not None) or (theta_tilt is not None):
                ...
                if self.ctr % 2 == 1:
                    ...
                    u,v,depth = self.get_pixel_depth(chest_cam_px, self.depth_img, z_replace=1.0)
                else:
                    theta_l_pan = -18
                    theta_r_pan = 18
                    theta_tilt = 22
            ...
            if self.ctr==self.num_ctr:
                ...
                results_dir = os.path.join(parent_dir, 'results','hanson_baseline_eval')
                ...
                title_str = 'hanson_baseline_eval'
...
if __name__ == '__main__':
    ...
    vismotor = VisuoMotorNode(num_trials=2000)
    rospy.spin()
